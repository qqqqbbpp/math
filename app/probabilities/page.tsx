            ...
            <main tabIndex={1}>
                <div className="main">
                    ...
                    <h2 id="1a">Классификация</h2>
                        <ol>
                            <li><p>Случайное&nbsp;&mdash; может произойти или не произойти в результате ичпытания <Latex>{`$A,B,\\ldots,Z$`}</Latex></p></li>
                            <li><p>Достоверные&nbsp;&mdash; обязательно произойдет в результате испытания</p></li>
                            <li><p>Невозможные&nbsp;&mdash; никогда не произойдет в результате ичпытания</p></li>
                    ...
                    <h2 id="1b">Операции над событиями</h2>
                        <ol>
                            <li><p>Сумма событий&nbsp;&mdash; события, состоящие в наступлении хотя бы одного из них в результате иcпытания.</p></li>
                    ...
                    <h2 id="1c">Вероятности</h2>
                        ...
                        <p>Статистическое определение предполагает, что число испытаний бесконечно. На првктике такой подход не работает, как правило колличество испытаний определено. При этом их колличество должно быть не мало. В этом случае возникает статистическая устойчивость результатов <Latex>{`$P=\\frac{m}{n}.$`}</Latex></p>
                    <h2 id="2">Комбинаторика</h2>
                        ...
                        <h3 id="2a">Формулы комбинаторики</h3>
                            <ol>
                                ...
                                <li>
                                    <p>Размещение из n элементов по m элементов <Latex>{`$A_n^m$`}</Latex>&nbsp;&mdash; это упорядоченный набор из m различных элементов, взятых из некоторого множества с мощностью n, где <Latex>{`$m\\leq n.$`}</Latex> То есть некая перестановка некоторых m выбраных элементов n</p>
                    ...
                    <h2 id="3">Теоремы</h2>
                        <h3 id="3a">Теормема сложения</h3>
                            ...
                            <p><Latex>{`$A_i$`}</Latex>&nbsp;&mdash; поппадание выстрелом <Latex>{`$i,$`}</Latex> где <Latex>{`$i= \\overline{1,3}.$`}</Latex> <Latex>{`$B$`}</Latex>&nbsp;&mdash; ровно одно попадание. <Latex>{`$B = A_1 \\times \\overline{A_2} \\times \\overline{A_3} + \\overline{A_1} \\times A_2 \\times \\overline{A_3} + \\overline{A_1} \\times \\overline{A_2} \\times A_3.$`}</Latex></p>
                            ...
                            <p>Для совметных событий: <Latex>{`$P(A+B)=P(A)+P(B)-P(A\\times B).$`}</Latex></p>
                        ...
                        <h3 id="3b">Теормема умножения</h3>
                            ...
                            <p>Методами математической индукции теормеу можно обособить для любого конечного числа событий:</p>
                            <p><Latex>{`$P \\prod\\limits_{i=1}^n (A_i)=\\prod\\limits_{i=1}^n (P(A_i))$`}</Latex></p>
                            <p>Пусть требуется определить аероятность некоторого события <Latex>{`$A$`}</Latex>, которое может произовйти вместе с одним из событий <Latex>{`$H_1, H_2, \\ldots, H_n$`}</Latex>&nbsp;&mdash; называемыми гипотезами.</p>
                        ...
                        <h3 id="3c">Теорема гипотез</h3>
                            ...
                            <p>Произведён опыт, в результате которого событие <Latex>{`$A$`}</Latex> произошло. Как следует изменить вероятности гипотез в свяхи с появлением события <Latex>{`$A?$`}</Latex> Необходимо найти вероятность гипотезы при условии, что событие <Latex>{`$A$`}</Latex> наступило.</p>
                    ...
                    <h2 id="4">Повторные независимые испытания</h2>
                        <p>При решении вероятностных задач часты ситуации в котороых одно и то же событие повторяется многократно (стрелок с места несколько раз стреляет в мишень), вопрос нахождения вероятности в отдельном испытании рассмотрен ранее. Представляет интерес нахождение вероятности появления конкретного события определенного числа в n-испытаниях.</p>
                        <p><Latex>{`$P_{m,n}(A)$`}</Latex>&nbsp;&mdash; ?</p>
                        <p>Несколько событий называют независимыми отностительно события <Latex>{`$A$`}</Latex> если вероятность события <Latex>{`$A$`}</Latex> в каждом из них не зависит от исхода других испытаний.</p>
                        ...
                        <h3 id="4a">Формула Бернулли</h3>
                            <p>Число различных комбинаций, в которых событи <Latex>{`$A$`}</Latex> наступает <Latex>{`$m$`}</Latex> раз из <Latex>{`$n$`}</Latex> возможных определяется формулой сочетания комбинаторики (формула Бернулли).</p>
                            <p><Latex>{`$P_{m,n}(A)=C_n^m\\times p^m \\times q^{n-m}$`}</Latex></p>
                            <p>30% изделий предприятия&nbsp;&mdash; высшего сорта, какова вероятность того, ятоиз 6 изделий предприятия 4&nbsp;&mdash; высшего сорта?</p>
                        ...
                        <h3 id="4b">Формула Пуассона</h3>
                            ...
                            <p>Вероятность появления на ферме ондатры альбиноса&nbsp;&mdash; 0.001. Найти вероятность того, что из 1000 выращеных животных <Latex>{`$<2$`}</Latex>&nbsp;&mdash; альбиносы.</p>
                        ...
                        <h3 id="4c">Формула Лапласа</h3>
                            ...
                            <p>При определенном технологическом процессе происходит 10 обрывов нити на 100 веретён в час, определить вероятность того, что в течении часа на 80 веретенах произойдет 7 надрывов нити.</p>
                    ...
                    <h2 id="5">Случайные величины</h2>
                        <p>На практике приходится сталкиваться с величинами, которые в результате опыта могут принимать различные заранее неизвестные значения (число солнечных дней, колличество полоэительных оценок в сессии, число пассажиров в маршрутке).</p>
                        ...
                        <h3 id="5b">Закон распределения случайной величины</h3>
                            ...
                            $`}</Latex></p>
                            <p>Определив все возможные значения случайной величины и правило нахождения соотвествующих вероятностей, можно получить полное представление о случайной величине.</p>
                        ...
                        <h3 id="6">Интегральная функция распределения</h3>
                            ...
                            <p>Если число <Latex>{`$x$`}</Latex> будет меняться произвольно то и вероятности выполнения неравенства <Latex>{`$X<x$`}</Latex> тоже будет меняться, следовательно вероятность есть функция аргумента. Эти функцию будем обозначать <Latex>{`$F(x) = P(X<x).$`}</Latex></p>
                            <p>Геометрически это вероятность того, что случайная точка на числовой оси будет расположена левее заданой точки <Latex>{`$x.$`}</Latex> Данная формула при исследовании вероятностей позволяет использовать апарат математического анализа.</p>
                        ...
                        <h3 id="7">Дифферинциальная функция распределения</h3>
                            <p>Дифферинциальная функция даёт полную характеристику как дискретной так и непрерывной случайной величины. В случае непрерывной случайной величины&nbsp;&mdash; это не единственнный способ задания закона.</p>					
                            ...
                            <h4 id="7a">Свойства</h4>
                                ...
                                <p>Выражение интегральной функции через дифферинциальную <Latex>{`$f(x), F(x)$`}</Latex>&nbsp;&mdash; взаимно определены, то есть знаю одну можно выразить другую</p>
                                <p><Latex>{`$F(x)=P(X<x)=P(-\\infty < X < x)=\\int\\limits_{-\\infty}^x f(x)dx$`}</Latex></p>
                        <h3 id="8">Числовые характеристики случайной величины</h3>
                            <p>Для решения многих практических задач совсем не обязательно знать все возможные значения случайной величины и соотвествующие вероятности. Достаточно указать отдельные числовые характеристики, отражающие важные черты закона распределения.</p>
                        <h3 id="8a">Математическое ожидание <Latex>{`$M(X)$`}</Latex></h3>
                            <p>Возможное задание случайной величины могут быть сосредоточены вокруг некоторого центра&nbsp;&mdash; среднего значения.</p>
                            ...
                            <h4 id="8a3">Свойства</h4>
                                <ol>
                                    ...
                                    <li><p><Latex>{`$M(constX)=constM(X)$`}</Latex></p></li>
                        ...
                        <h3 id="8b">Дисперсия <Latex>{`$D(X)$`}</Latex></h3>
                            <p>Целесообразно находить меру рассеивания значений случайной величины вокруг её математичемкого ожидания.</p>
                            ...
                            <h4 id="8b3">Свойства</h4>
                                <ol>
                                    ...
                                    <li><p><Latex>{`$D(constX)=const^2D(X)$`}</Latex></p></li>
                    ...
                    <h2 id="9">Конкретные законы распределения</h2>
                        <p>Конкретным называется закон дифферинциальная функция, которого исселедована и найдены формулы для вычисления числовых характеристик.</p>
                        ...
                        <h3 id="9c">Нормальное распределение</h3>
                            ...
                            <ol>
                                ...
                                <li><p><Latex>{`$\\lim\\limits_{X\\to \\infty} f(x) = 0, ox$`}</Latex>&nbsp;&mdash; горизантальная асимтота</p></li>
                            ...
                            <p>На практике использвание формул <Latex>{`$f(x), F(x)$`}</Latex>&nbsp;&mdash; затруднительно, поэтому для решения практических задач полагают <Latex>{`$a=0, ~\\sigma = 1$`}</Latex></p>
                    ...
                    <h2 id="10">Предельные теоремы теории вероятности</h2>
                        <p>Результаты отдельных наблюдений при относсительно равенстве условий их проведения всё же отличаются друг от друга, хотя средний результат обнаруживает значительную устойчивость. Условия при которых проявляется такая закономерность рассматриваются в предельных теоремах.</p>
                        <h3 id="10a">Классификация</h3>
                            <ol>
                                <li><p>Закон больших чисел</p>
                                    <ol className="lower-latin">
                                        <li><p>Неравенство Маркова</p></li>
                                        <li><p>Неравенство Чебышева</p></li>
                                        <li><p>Теорема Чебышева</p></li>
                                        <li><p>Теорема Бернулли</p></li>
                                        <li><p>Теорема Пуассона</p></li>
                                    </ol>
                                </li>
                                <li><p>Центральная предельная теорема</p></li>
                            </ol>
                        <h3 id="10b">Неравенство Маркова</h3>
                            <p>Если <Latex>{`$X$`}</Latex> принимает только не отрицательные значения и имеет конечтое математическое ожидание, то имеет место неравенство:</p>
                            <p><Latex>{`$P(X\\leq\\xi)> 1 - \\frac{M(X)}{\\xi}$`}</Latex></p>
                            <p><Latex>{`$P(X>\\xi)<\\frac{M(X)}{\\xi}$`}</Latex></p>
                        <h3 id="10c">Неравенство Чебышева</h3>
                            <p><Latex>{`$P(|X-M(X)|\\leq\\xi)>1 - \\frac{D(X)}{\\xi^2}$`}</Latex></p>
                        <h3 id="10d">Теорема Чебышева</h3>
